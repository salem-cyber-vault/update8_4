...
const GoogleCustomSearchPanel: React.FC<GoogleCSEProps> = ({ apiKey, cseId }) => {
  ...
  const fetchResults = async (searchQuery: string, index: number = 1) => {
    ...
    try {
      const res = await fetch(
        `https://www.googleapis.com/customsearch/v1?key=${apiKey}&cx=${cseId}&q=${encodeURIComponent(searchQuery)}&start=${index}`
      );
  ...
  // Highlight search terms in snippet/title
  ...
  return (
    <div className="bg-zinc-900 text-zinc-100 rounded-lg p-6 shadow-lg">
      ...
      <style>{`
        ...
        .gcse-card:hover {
          box-shadow: 0 4px 16px 0 #6d28d9a0;
...
export default GoogleCustomSearchPanel;
